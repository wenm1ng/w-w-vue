<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column width="150px" align="center" label="所属部门">
        <template slot-scope="{ row }">
          <span>{{ row.departments.name }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    adminDelUniId(id){
      this.$base.confirm(
        { content: "确定要清除此账号的设备mac吗" },
        () => {
          adminDelUniId({ id: id }).then((response) => {
            if (response.status === 20000) {
              this.$base.message({ message: response.message });
            }
          });
