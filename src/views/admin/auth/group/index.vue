<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input v-model="listQuery.name" placeholder="请输入请输入店铺名称" style="width: 200px;" class="filter-item" clearable @change="handleFilter"/>
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column width="160px" align="center" label="资源库级别">
        <template slot-scope="{row}">
    ...
    <el-dialog   title="添加" :visible.sync="addDialogVisible" width="80%" @close="addDialogClose">
      ...
      <el-form label-width="100px" :model="addForm" :rules="addRules" ref="addRef">
        ...
        <el-form-item label="资源库权限等级">
          <el-select v-model="addForm.level" placeholder="权限等级从高到低依次递减" filterable clearable style="width:100%">
            <el-option v-for="item in [{'id':1,'name':'1级'},{'id':2,'name':'2级'},{'id':3,'name':'3级'},{'id':4,'name':'4级'},{'id':5,'name':'5级'}]" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
    ...
    <el-dialog   title="编辑" :visible.sync="editDialogVisible" width="80%" @close="editDialogClose">
      <!-- 主体区 -->
      <el-form label-width="150px" :model="editForm" :rules="addRules" ref="editRef">
        <el-form-item label="权限组名称" prop="name">
        ...
        <el-form-item label="资源库权限等级">
          <el-select v-model="editForm.level" placeholder="请选择资源库权限等级" filterable clearable style="width:100%">
            <el-option v-for="item in [{'id':1,'name':'1级'},{'id':2,'name':'2级'},{'id':3,'name':'3级'},{'id':4,'name':'4级'},{'id':5,'name':'5级'}]" :key="item.id" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['编号', '权限组名称','描述','创建时间','编辑时间','状态','素材库级别']
        const filterVal = ['id', 'name','content','updated_at','created_at','status','level']
